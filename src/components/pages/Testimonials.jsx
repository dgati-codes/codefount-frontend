  ...
  return (
    <div className="test-stars">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} size={13} fill={i < n ? '#f59e0b' : 'none'} color={i < n ? '#f59e0b' : '#cbd5e1'} />
      ))}
    </div>
  );
}
    ...
    <div className="page-enter">
      ...
      <section className="test-hero">
        <div className="container">
          ...
          <h1 className="test-hero__h1">Stories from Our <span>Alumni</span></h1>
          <p className="test-hero__sub">
            Over 10,000 graduates across Ghana, Nigeria, UK and beyond. Here's what some of them say.
          </p>
        </div>
      </section>
      ...
      <section className="section-sm">
        <div className="container">
          <div className="test-featured">
            <button className="test-arrow test-arrow--l" onClick={prev}><ChevronLeft size={20}/></button>
            ...
            <div className="test-featured-card">
              ...
              <div className="test-featured-footer">
                ...
                <div>
                  <div className="test-name">{featured.name} {featured.country}</div>
              ...
              <div className="test-dots">
                {testimonials.map((_, i) => (
                  <button key={i} className={`test-dot ${i===current?'test-dot--on':''}`} onClick={() => setCurrent(i)}/>
                ))}
              </div>
            </div>

            <button className="test-arrow test-arrow--r" onClick={next}><ChevronRight size={20}/></button>
          </div>
      ...
      <section className="section" style={{ background:'var(--bg-soft)', paddingTop:'2rem' }}>
        <div className="container">
          <h2 className="h2" style={{ textAlign:'center', marginBottom:'2rem' }}>
            What Our Students <span>Say</span>
          </h2>
          <div className="test-grid">
            {testimonials.map((t, i) => (
              <div key={t.id} className={`test-card ${i === current ? 'test-card--active' : ''}`}
                   onClick={() => setCurrent(i)}>
                <Stars n={t.rating}/>
                <p className="test-card-text">"{t.text.slice(0, 140)}{t.text.length > 140 ? '…' : ''}"</p>
                <div className="test-card-footer">
                  <div className="test-avatar test-avatar--sm">{t.avatar}</div>
                  <div>
                    <div className="test-name test-name--sm">{t.name} {t.country}</div>
          ...
          <div className="test-cta">
            ...
            <div style={{ display:'flex', gap:'1rem', justifyContent:'center', flexWrap:'wrap' }}>
              ...
              <a href="/contact"  className="btn btn-outline btn-lg">Talk to an Advisor</a>
            </div>
          </div>
        </div>
      </section>
